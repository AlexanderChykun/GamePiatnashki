    ...
    public class UI
    {
        ...
        public static void ShowField(Field f) 
        {
            for (int i = 0; i < f.GetCol(); i++)
            {
                ...
                {
                    UI.ShowCell(i * 5 + 25, j * 4 + 3, f.GetSymbol(i, j));
                }
            }
            Console.WriteLine();
        }
        ...
        /// <summary>
        /// Отображение надписи о неправильном ходе
        /// </summary>
        /// <param name="size">размер игрового поля</param>
        /// <param name="defSize">размер игрового поля по-умолчанию</param>
        public static void WrongWay(int size, int defSize) 
        {
            const int posX = 15;          //Стартовые координаты расположения сообщения                  
            const int posY = 20;
            ...
            Console.WriteLine("Неправильный ход!!! Повторите свой ход.");
            Thread.Sleep(500);
        }
        /// <summary>
        /// Счетчик ходов
        ...
        public static void Counter ( object sender, MovingEventArgs arg )
        {
            const int posX = 26;                                //Стартовые координаты расположения счетчика                  
            const int posY = 20;
        ...
        public static void GetUserAction ( Field f , ref bool newGame, ref bool cont)
        {
